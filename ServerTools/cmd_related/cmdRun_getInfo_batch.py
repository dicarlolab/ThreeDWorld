...
if __name__ == "__main__":
    ...
    parser.add_option("-w", "--windowsflag", dest="windowsflag", default = 0, type=int) #Not supported yet
    ...
    if options.windowsflag==0:
        ...
        txtout_path     = '/home/chengxuz/test_empty_all/text_all'
        ...
        output_path     = 'Assets/PrefabDatabase/Info_for_' + str(options.startn) + '_' + str(options.lengthn) + '.txt'
    else:
        # Not supported yet
    ...
    target_prefix   = project_path + '/Assets/PrefabDatabase/AssetBundles/bundle_part/'
    ...
    for index_tmp in range(options.startn, max_lengthn):
        ...
        new_path    = os.path.join(bundle_path, shapenet_info_tmp['id'] + '.bundle')
        #print(new_path)
        os.system('cp ' + new_path + ' ' + target_prefix)
    ...
    cmd_str         = unity_path + ' -batchmode -quit -projectPath ' + project_path + ' -executeMethod CreatePrefabCMD.LoadInfoFromBundle -nographics -inputDir Assets/PrefabDatabase/AssetBundles/bundle_part/ -outputFile ' + output_path
    print(cmd_str)
    os.system(cmd_str)
    os.system('mv ' + os.path.join(project_path , output_path) + ' ' + txtout_path)
