...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  conv1_weights = tf.Variable(
      tf.truncated_normal([7, 7, NUM_CHANNELS, 32],
                          stddev=0.01,
                          seed=SEED),
      name = 'conv1w' )
  ...
  fc1_weights = tf.Variable(
      tf.truncated_normal(
          [IMAGE_SIZE//4 * IMAGE_SIZE//4 * 32, 256],
          stddev=0.01,
          seed=SEED), name='fc1w')
  fc1_biases = tf.Variable(tf.constant(0.01, shape=[256]), name='fc1b')
  ...
  def model(data, train=False):
    ...
    flatten = tf.reshape(pool1, [pool1_shape[0], np.prod(pool1_shape[1:])])
    ...
    conv = tf.nn.conv2d(unpool,
                    conv6_weights,
                        strides=[1, 1, 1, 1],
                        padding='SAME')
    conv = tf.nn.bias_add(conv, conv6_biases)

    return conv

  train_prediction = model(train_data_node, True)  
  loss = tf.nn.l2_loss((train_prediction - train_data_node) / (IMAGE_SIZE*IMAGE_SIZE*3))
  ...
  #optimizer = tf.train.AdamOptimizer(leaning_rate).minimize(loss, global_step=batch)
