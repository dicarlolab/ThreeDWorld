...
def getNextBatch(N, start):
  ...
  for i in range(N):
    timestep = start + i
    info, nstr, ostr, imstr = handle_message(sock, write=True, outdir='normal_encoder_simple2', prefix=str(i))
  ...
  with tf.Session() as sess:
    ...
    for step in xrange(NUM_TRAIN_STEPS // BATCH_SIZE):
      ...
      _, l, lr, predictions = sess.run(
          ...
          feed_dict=feed_dict)
      print(step, l, lr)
      np.save('normal_encoder_simple2/prediction.npy', predictions)
