...
def getEpisode(epnum):
  ...
  while timestep < EPISODE_LENGTH + 1:
    pf = str(timestep)
    info, nstr, ostr, imstr = handle_message(sock, write=True, outdir='sequences_nt2', prefix=pf)
    ...
    if first or len(objs) == 0:
      ...
      if not chosen:
        ...
        o = objarray[x, y]
        force = [rng.choice([-20, 0, 20]), 40, rng.choice([-40, 0, 20])]
        chosen = True
      ...
      #if timestep % 10 == 0:
      #  msg['msg']['vel'] = [.05 * rng.uniform(), 0.025 * rng.uniform(), 0.05 * rng.uniform()]
...
def main(argv):
  ...
  with tf.Session() as sess:
    ...
    for episode in xrange(NUM_EPISODES):
      obs = []; acts = []; td = []; fut = []
      for _k in range(NUMSS):
          b = getNextBatch(BATCH_SIZE // NUMSS, NUMSS * episode + _k)
          obs.append(b['observations'])
          acts.append(b['actions'])
          td.append(b['time_diff'])
          fut.append(b['future'])
      obs = np.row_stack(obs)
      acts = np.row_stack(acts)
      td = np.concatenate(td)
      fut = np.row_stack(fut)
                  
      feed_dict = {observation_node: obs,
                   actions_node: acts, 
                   time_node: td[:, np.newaxis], 
                   future_node: fut}
      ...
      _, l, lr, predictions = sess.run(
          ...
          feed_dict=feed_dict)
      print(episode, l, lr)
      np.save('sequences_nt2/prediction.npy', predictions)
