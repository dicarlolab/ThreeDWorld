...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  fc2_weights = tf.Variable(  # fully connected, depth 512.
      tf.truncated_normal(
          ...
          stddev=0.01,
          seed=SEED), name='fc2w')
  fc2_biases = tf.Variable(tf.constant(0.01, shape=[48 * 32 * 32]), name='fc2b')
  ...
  def model(data, train=False):
    ...
    unflatten = tf.reshape(hidden, [hidden_shape[0], IMAGE_SIZE//8, IMAGE_SIZE//8, 48])
    ...
    unpool = tf.image.resize_images(conv4, IMAGE_SIZE//4, IMAGE_SIZE//4)
