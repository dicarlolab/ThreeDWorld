...
def model(current, future, actions_node, time_node, rng, cfg):
  ...
  decode = tf.concat(3, [decode, encode_nodes_current[encode_depth]])
  ...
  loss = tf.nn.l2_loss(decode - encode_nodes_future[encode_depth]) / norm
  for i in range(1, encode_depth + 1):
    nf0 = nf
    ...
    cfs = cfg0['encode'][encode_depth - i]['conv']['filter_size']
    nf = encode_nodes_future[encode_depth - i].get_shape().as_list()[-1]
    decode = tf.concat(3, [decode, encode_nodes_current[encode_depth - i]])
    W = tf.Variable(tf.truncated_normal([cfs, cfs, 2 * nf0, nf],
                                        stddev=0.1,
                                        seed=fseed))
    b = tf.Variable(tf.zeros([nf]))
    decode = tf.nn.conv2d(decode,
    ...
    norm = (ds**2) * BATCH_SIZE * nf
    loss = loss + tf.nn.l2_loss(decode - encode_nodes_future[encode_depth - i]) / norm
