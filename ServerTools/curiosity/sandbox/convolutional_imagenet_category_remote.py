...
HOST = '18.93.6.11'
PORT = 23046
PATH = '/data/imagenet_dataset/hdf5_cached_from_om7/data.raw'
KEYS = ['data', 'labels']
...
def make_req(type, batch_num):
    """
    Creates the zmq request to be sent to server (hdf5_handler).
    :type Type of request {'batch', 'size'}
    :param batch_num: current batch number
    :return: request string
    """
    if type == 'batch':
      req = {'path': PATH, 'keys': KEYS, 'batch_size': BATCH_SIZE, 'batch_num': batch_num}
    elif type == 'size':
      req = {'path': PATH, 'keys': KEYS, 'size': True}
    return req
...
def iterate_minibatches(socket, slice):
  """
  Iterates through batches created over slice.
  :param socket: ZMQ socket to access server
  :param slice: Binary array indicating the indices that should be iterated over.
  :return: data consisting of all keys in KEYS
  """
  num_batches = np.floor(np.count_nonzero(slice) / float(BATCH_SIZE)).astype(int)
  baseline = np.ceil(np.min(slice.nonzero()) / float(BATCH_SIZE)).astype(int)

  for cur_batch_num in range(baseline, baseline+num_batches):
    yield receive_data(socket, 'batch', cur_batch_num)

def main(argv=None):  # pylint: disable=unused-argument
    # Get the data.

  ## What does preprocess???
  norml = lambda x: (x - (PIXEL_DEPTH/2.0)) / PIXEL_DEPTH
  preprocess_data = lambda x: norml(x).reshape((x.shape[0], 3, 256, 256)).swapaxes(1, 2).swapaxes(2, 3)
  ...
  # Populate validation data and labels on memory
  validation_data = []
  validation_labels = []
  for step, batch in enumerate(iterate_minibatches(socket, validation_slice)):
    validation_data.append(batch['data'])
    validation_labels.append(batch['labels'])
  validation_data = np.row_stack(validation_data)
  validation_data = preprocess_data(validation_data)    # Normalize and reshape the examples
  validation_labels = preprocess_lables(np.concatenate(validation_labels))

  # Populate test data and labels on memory
  test_data = []
  test_labels = []
  for step, batch in enumerate(iterate_minibatches(socket, test_slice)):
    test_data.append(batch['data'])
    test_labels.append(batch['labels'])
  test_data = np.row_stack(test_data)
  test_data = preprocess_data(test_data)    # Normalize and reshape the examples
  test_labels = preprocess_lables(np.concatenate(test_labels))

  num_epochs = NUM_EPOCHS
  # train_size = receive_data(socket, 'size', 0)['size']
  train_size = np.count_nonzero(train_slice)
  ...
  with tf.Session() as sess:
    ...
    for epoch in range(num_epochs):
      ...
      for step, batch in enumerate(iterate_minibatches(socket, train_slice)):
        batch_data = preprocess_data(batch['data'])   # Normalize and reshape the examples
        batch_labels = preprocess_lables(batch['labels'])
