    ...
    if not valid[start: end].all():
        ...
        for i in range(bsize):
            info, narray, oarray, imarray = handle_message(sock, 
                                                           write=True, 
                                                           outdir='/data2/datasource4_ims', prefix=str(i))
            msg = {'n': 4,
            ...
            if i == 0:
                ...
                if len(obs) == 0:
                    ...
                    if not chosen or chosen_o not in obs:
                        chosen_o = choose(obs[np.argsort(fracs)[-3:]])
                        chosen = True
                    ...
                    print('FRAC:', max(fracs), frac0)
                    if max(fracs) < .02 and not action_started:
                        msg['msg']['vel'] = [0, 0, .25]
                    else:
                        action = {}
