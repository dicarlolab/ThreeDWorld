...
def getNextBatch(N, start):
  ...
  for i in range(N):
    timestep = start + i
    info, nstr, ostr, imstr = handle_message(sock, write=True, outdir='normals_autoencoder', prefix=str(i))
...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  def model(data, train=False):
    ...
    print('conv1a', conv.get_shape().as_list())
    ...
    print('conv1b', conv.get_shape().as_list())
    ...
    print('encode', encode.get_shape().as_list())
    ...
    print('unflatten', unflatten.get_shape().as_list()) 
    unpool = tf.image.resize_images(unflatten, IMAGE_SIZE, IMAGE_SIZE)
    print('unpool', unpool.get_shape().as_list())
  ...
  with tf.Session() as sess:
    ...
    for step in xrange(NUM_TRAIN_STEPS // BATCH_SIZE):
      ...
      feed_dict = {train_data_node: batch_data['normals'],
      ...
      _, l, lr, predictions = sess.run(
          ...
          feed_dict=feed_dict)
      print(step, l, lr)
      np.save('normals_autoencoder/prediction.npy', predictions)
