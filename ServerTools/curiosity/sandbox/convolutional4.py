...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  fc1_weights = tf.Variable(  # fully connected, depth 512.
      tf.truncated_normal(
          ...
          stddev=0.1,
          seed=SEED), name='fc1w')
  fc1_biases = tf.Variable(tf.constant(0.1, shape=[64]), name='fc1b')
  ...
  fc2_weights = tf.Variable(  # fully connected, depth 512.
      tf.truncated_normal(
          ...
          stddev=0.1,
          seed=SEED), name='fc1w')
  fc2_biases = tf.Variable(tf.constant(0.1, shape=[32 * 14 * 14]), name='fc1b')
  ...
  def model(data, train=False):
    ...
    unflatten = tf.reshape(hidden, [hidden_shape[0], 14, 14, 32])

    unpool = tf.image.resize_images(unflatten, 28, 28)
